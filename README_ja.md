...
> **ヒント：** 最終レンダリングには **Freestyle (Quality)** アウトラインを推奨します。メッシュのトポロジに沿った、よりきれいな線画が得られます。Solidify はプレビュー向けです。
...
開発過程で **Claude モデルは使用していません**。**v0.5.1 以前**の一部の開発でのみ、**Claude Code** ツール（サードパーティモデルを接続）を使用しました。  
